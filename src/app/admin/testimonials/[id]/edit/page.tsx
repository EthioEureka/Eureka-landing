    ...
    async function loadTestimonial() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.testimonial) {
            const t: Testimonial = data.testimonial;
            setFormData({
              client_name: t.client_name || "",
              role: t.role || "",
              company: t.company || "",
              quote: t.quote || "",
              photo: t.photo || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80",
              featured: !!t.featured,
  ...
  if (loading) {
    return (
      <div className="py-20 flex items-center justify-center text-eureka-slate font-mono text-xs">
        <Loader2 className="animate-spin mr-2 text-eureka-blue" size={16} /> Loading testimonial details...
      </div>
    );
  }
    ...
    <div className="space-y-8 max-w-4xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-eureka-dark tracking-tight">
              Edit Testimonial: {formData.client_name}
            </h1>
            <p className="text-xs font-mono text-eureka-slate mt-1 font-semibold">
              Update endorsement quote, executive details, and avatar image
            </p>
          </div>
        </div>
      </div>
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 font-mono text-xs flex items-center gap-2 rounded-xl font-bold">
          <Sparkles size={16} className="text-emerald-600" /> Testimonial updated successfully. Redirecting to list...
        </div>
      )}
